#!/usr/bin/env python3
"""
Script d'ingestion pour ChromaDB en mode persistant local
Évite les problèmes d'incompatibilité d'API en accédant directement aux données
"""
import os
import json
import glob
...
CHROMA_PATH = "/chroma/chroma"  # Chemin vers les données persistantes de ChromaDB
...
async def main():
    ...
    print("🔍 Initialisation de ChromaDB en mode persistant...")

    # Créer le client ChromaDB en mode persistant
    ...
    print(f"✅ ChromaDB initialisé (path: {CHROMA_PATH})")

    # Créer ou récupérer la collection
    try:
        collection = client.get_collection(name=COLLECTION_NAME)
        count = collection.count()
        print(f"ℹ️  Collection '{COLLECTION_NAME}' existe déjà ({count} documents)")

        # Supprimer et recréer pour recommencer
        ...
        print(f"ℹ️  Collection n'existe pas encore: {e}")
        collection = client.create_collection(
            name=COLLECTION_NAME,
            metadata={"description": "Knowledge base for MY-IA RAG system"}
        )
    ...
    # Test de recherche
    print("\n🔍 Test de recherche...")
    test_query = "Comment installer MY-IA?"
